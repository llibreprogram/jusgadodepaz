...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.responsive = get_responsive_helper()  # Initialize responsive helper
        self.filtered_cases = []
        self.current_page = 1
    ...
    def resizeEvent(self, event):
        ...
        # Update responsive helper metrics (in case window moved to different screen)
        self.responsive.refresh_metrics()
        
        # Note: Graphs will be redrawn when their tab is activated
        # This prevents unnecessary redraws when user is on a different tab
    ...
    def init_ui(self):
        ...
        container_layout.setContentsMargins(margins(), margins(), margins(), margins())
        ...
        self.tabs.addTab(self.create_register_tab(), "Registrar Caso")
        self.tabs.addTab(self.wrap_in_scroll(self.create_view_tab()), "Ver Casos")
        self.tabs.addTab(self.wrap_in_scroll(self.create_stats_tab()), "Dashboard")
        self.tabs.addTab(self.wrap_in_scroll(self.create_export_tab()), "Exportar")
        self.tabs.addTab(self.wrap_in_scroll(self.create_import_tab()), "Importar")
        self.tabs.addTab(self.wrap_in_scroll(self.create_alerts_tab()), "Alertas")
        self.setup_autocompleters()

    def wrap_in_scroll(self, widget):
        """Wrap a widget in a QScrollArea for low resolution support"""
        scroll = QScrollArea()
        scroll.setWidgetResizable(True)
        scroll.setWidget(widget)
        scroll.setFrameShape(QFrame.Shape.NoFrame)
        # Transparent background for scroll area
        scroll.setStyleSheet("QScrollArea { background: transparent; border: none; } QWidget#scroll_content { background: transparent; }")
    ...
    def apply_theme(self):
        ...
        border = '#1e293b'
        text = '#e5e7eb'
        # Scaled values for CSS
        fs_base = scale_font(14)
        fs_small = scale_font(13)
        fs_header = scale_font(20)
        pad_base = scale(12)
        pad_wide = scale(20)
        ...
        self.setStyleSheet(f"""
            ...
            QTabBar::tab {{
                background: {soft}; 
                color: {text}; 
                padding: {pad_base}px {pad_wide}px; 
                margin: {scale(4)}px;
                border-radius: 10px; 
            ...
            QCheckBox {{ color: {text}; font-size: {fs_base}px; }}
            QScrollBar:vertical {{ 
                background: {surface}; 
                width: 12px;
                border-radius: 6px;
    ...
    def create_register_tab(self):
        ...
        self.departamento_actual.addItems([
            '', 
            'Sede Policial',
            'Sistema de Atención (UAVVG)', 
            'Sistema de Atención Esperanza',
            'Sistema de Atención Mao',
            'Sistema de Atención Laguna Salada'
        ])
        ...
        def create_date_field_with_clear(date_widget):
            ...
            clear_btn.setMinimumSize(scale(40), scale(28))
            clear_btn.setMaximumSize(scale(60), scale(36))
    ...
    def create_view_tab(self):
        """Create view cases tab with filters and table"""
        tab = QWidget()
        layout = QVBoxLayout()
        ...
        self.search_input.setMinimumHeight(scale(32))
        self.search_input.setPlaceholderText('Buscar carpeta, nombres, etc.')

        self.filter_categoria = QComboBox()
        self.filter_categoria.setMinimumHeight(scale(32))
        self.filter_categoria.addItems(['Todas', '─── Generales ───', 'Pensión alimentaria (todas)', 'Tránsito (todas)', '─── Específicas ───'] + self.categories)
        self.filter_categoria.setCurrentIndex(0)

        self.filter_etapa = QComboBox()
        self.filter_etapa.setMinimumHeight(scale(32))
        self.filter_etapa.addItems(self.filter_etapas)

        self.filter_estado = QComboBox()
        self.filter_estado.setMinimumHeight(scale(32))
        ...
        self.filter_fiscal.setMinimumHeight(scale(32))
        ...
        # Grid layout for filters to save vertical space
        filters_grid = QGridLayout()
        filters_grid.setSpacing(spacing())
        
        # Row 0: Search input (span all)
        filters_grid.addWidget(QLabel("🔍 Búsqueda:"), 0, 0)
        filters_grid.addWidget(self.search_input, 0, 1, 1, 5)
        
        # Row 1: Category, Stage, State
        filters_grid.addWidget(QLabel("📁 Cat:"), 1, 0)
        filters_grid.addWidget(self.filter_categoria, 1, 1)
        filters_grid.addWidget(QLabel("⚖️ Etapa:"), 1, 2)
        filters_grid.addWidget(self.filter_etapa, 1, 3)
        filters_grid.addWidget(QLabel("📝 Est:"), 1, 4)
        filters_grid.addWidget(self.filter_estado, 1, 5)
        
        # Row 2: Fiscal, Dates, Checkbox
        filters_grid.addWidget(QLabel("👤 Fiscal:"), 2, 0)
        filters_grid.addWidget(self.filter_fiscal, 2, 1)
        filters_grid.addWidget(QLabel("📅 Desde:"), 2, 2)
        filters_grid.addWidget(self.filter_fecha_desde, 2, 3)
        filters_grid.addWidget(QLabel("hasta:"), 2, 4)
        filters_grid.addWidget(self.filter_fecha_hasta, 2, 5)
        filters_grid.addWidget(self.filter_apelados, 2, 6)

        search_button = QPushButton('🔍 Buscar')
        search_button.setMinimumHeight(scale(34))
        search_button.clicked.connect(self.search_cases)
        
        clear_filters_button = QPushButton('🔄 Limpiar filtros')
        clear_filters_button.setMinimumHeight(scale(34))
        clear_filters_button.clicked.connect(self.clear_filters)
        clear_filters_button.setStyleSheet("""
            QPushButton { background: #ef4444; color: white; border: none; padding: 6px 14px; border-radius: 6px; font-weight: 600; }
        ...
        search_card_layout.setContentsMargins(margins(), margins(), margins(), margins())
        search_card_layout.setSpacing(spacing())
        
        search_card_layout.addLayout(filters_grid)
        
        search_btns = QHBoxLayout()
        search_btns.setSpacing(spacing())
        search_btns.addStretch()
        search_btns.addWidget(search_button)
        search_btns.addWidget(clear_filters_button)
        search_card_layout.addLayout(search_btns)
        
        search_card.setLayout(search_card_layout)
        layout.addWidget(search_card)

        self.table = QTableWidget()
        # Ensure table has enough height in scroll area
        self.table.setMinimumHeight(scale(400))
        ...
        # Configure column resize modes for better visibility
        header = self.table.horizontalHeader()
        
        # Use a more reliable approach for column widths
        # First, set a base font to calculate metrics
        font = self.table.font()
        metrics = QFontMetrics(font)
        
        # Calculate width for "Categoría/Delito" + some padding
        cat_width = metrics.horizontalAdvance("Categoría/Delito") + scale(60)
        
        # Set modes
        header.setSectionResizeMode(QHeaderView.ResizeMode.Interactive)
        self.table.setColumnWidth(2, cat_width)
        
        # Adjust other columns based on content but with a minimum
        header.setMinimumSectionSize(scale(80))
        
        # Stretch columns that contain long user data
        header.setSectionResizeMode(4, QHeaderView.ResizeMode.Stretch)  # Víctima
        header.setSectionResizeMode(5, QHeaderView.ResizeMode.Stretch)  # Investigado
        
        # All others to ResizeToContents for now
        for i in range(17):
            if i not in [2, 4, 5]:
        ...
        self.prev_page_btn.setMinimumWidth(scale(100))
        self.prev_page_btn.setMaximumWidth(scale(150))
        self.prev_page_btn.setMinimumHeight(36)
        ...
        self.next_page_btn.setMinimumWidth(scale(100))
        self.next_page_btn.setMaximumWidth(scale(150))
        ...
        self.items_per_page_combo.setMinimumWidth(scale(70))
        self.items_per_page_combo.setMaximumWidth(scale(100))
        ...
        docs_button.setMinimumHeight(40)
    ...
    def create_export_tab(self):
        ...
        case_selector_label.setWordWrap(True)
        self.export_case_combo = QComboBox()
        self.export_case_combo.setMinimumHeight(36)
        ...
        export_single_csv_btn.setMinimumHeight(40)
        ...
        export_single_excel_btn.setMinimumHeight(40)
        ...
        export_csv_filtered_btn.setMinimumHeight(40)
        ...
        export_excel_filtered_btn.setMinimumHeight(40)
    ...
    def _metric_card(self, title: str, value, detail: str):
        ...
        vbox = QVBoxLayout()
        vbox.setContentsMargins(scale(10), scale(8), scale(10), scale(8))
        vbox.setSpacing(scale(4))

        title_lbl = QLabel(title)
        title_lbl.setStyleSheet(f"font-size: {scale_font(12)}px; color: #94a3b8;")
        value_lbl = QLabel(str(value))
        value_lbl.setStyleSheet(f"font-size: {scale_font(20)}px; font-weight: 800; color: #60a5fa;")
        detail_lbl = QLabel(detail)
        detail_lbl.setStyleSheet(f"font-size: {scale_font(11)}px; color: #cbd5e1;")
    ...
    def create_import_tab(self):
        ...
        header_layout.setContentsMargins(margins(), margins(), margins(), margins())
        ...
        # Use metrics to ensure buttons are wide enough for the text
        btn_font = QFont('Segoe UI', 10)
        btn_metrics = QFontMetrics(btn_font)
        
        excel_text = '⬇️ Descargar Plantilla Excel'
        csv_text = '⬇️ Descargar Plantilla CSV'
        excel_w = btn_metrics.horizontalAdvance(excel_text) + scale(80)
        csv_w = btn_metrics.horizontalAdvance(csv_text) + scale(80)
        
        download_excel_btn = QPushButton(excel_text)
        download_excel_btn.setMinimumHeight(35)
        download_excel_btn.setMinimumWidth(excel_w)
        download_excel_btn.setSizePolicy(QSizePolicy.Policy.Preferred, QSizePolicy.Policy.Fixed)
        download_excel_btn.clicked.connect(lambda: self.download_template('excel'))
        
        download_csv_btn = QPushButton(csv_text)
        download_csv_btn.setMinimumHeight(35)
        download_csv_btn.setMinimumWidth(csv_w)
        download_csv_btn.setSizePolicy(QSizePolicy.Policy.Preferred, QSizePolicy.Policy.Fixed)
        ...
        self.import_file_path.setMinimumHeight(36)
        ...
        # Calculate width using metrics
        metrics = QFontMetrics(browse_btn.font())
        browse_w = metrics.horizontalAdvance('🔍 Examinar') + scale(40)
        browse_btn.setMinimumWidth(browse_w)
        ...
        self.skip_duplicates_check.setStyleSheet(f'font-size: {scale_font(13)}px; color: #cbd5e1; margin-top: {scale(8)}px;')
        
        validate_text = '✓ Validar Archivo'
        validate_btn = QPushButton(validate_text)
        validate_btn.setMinimumHeight(40)
        validate_btn.clicked.connect(self.validate_import_file)
        
        # Calculate width for validate button
        validate_w = metrics.horizontalAdvance(validate_text) + scale(60)
        validate_btn.setMinimumWidth(validate_w)
    ...
    def refresh_alerts(self):
        ...
        try:
            ...
            for row, alert in enumerate(alerts):
                ...
                action_btn.setMinimumSize(scale(100), scale(30))
                action_btn.setMaximumSize(scale(140), scale(40))
    ...
    def open_documents_dialog(self):
        ...
        for row, doc in enumerate(documents):
            ...
            open_btn.setMinimumSize(scale(28), scale(24))
            open_btn.setMaximumSize(scale(40), scale(32))
            open_btn.setToolTip('Abrir documento')
            open_btn.clicked.connect(lambda checked, d=doc_id: self.open_document(d))
            
            delete_btn = QPushButton('🗑️')
            delete_btn.setMinimumSize(scale(28), scale(24))
            delete_btn.setMaximumSize(scale(40), scale(32))
